...
class discrete_REDQ(nn.Module):
    ...
    def collect_trajectories_other_policy(self,other_agent_policy_network:nn.Module,env:gym.Env) -> int:
        """
        (s,s',a,r,done) from other policy network will be saved to the experience buffer 
        """
        state,_ = env.reset()
        cummulative_reward = 0 
        ...
        with torch.no_grad():
            ...
            while(True):
                _,dist = other_agent_policy_network(torch.tensor(np.array(state),dtype=torch.float,device=self.device).unsqueeze(dim=0))
                action = dist.sample().item()
                next_state, reward, done, _, _ = env.step(action)
                self.buffer.append((state,next_state,action,reward,done))
...
"""
will collect a new buffer using a previous checkpoint 
...
if __name__ == "__main__":
    ...
    rollout_ever_n_steps = 300
    epsilon = 0.0 #rerun with 0 epsilon per trace  # 0.3

    num_policies_cloned = 300
    ...
    #setup and load previous parameters
    policy_net = policy_network(obs_size,action_size,policy_body_struct).to(device=device)
    file_name = "./checkpoints_old2/42400.pt"
    parameters = torch.load(file_name)
    policy_net.load_state_dict(parameters)
    
    #collect old trajectories from policy
    for clone_step in tqdm(range(num_policies_cloned),ascii=True,desc="clone_policy"):
        
        cummulative_reward = REDQ.collect_trajectories_other_policy(policy_net,env)
        writer.add_scalar("collected_previous_policy",cummulative_reward,clone_step)
